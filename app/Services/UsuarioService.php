...
final class UsuarioService
{
    public function validar(array $entrada, ?int $usuarioId = null, bool $respuestasOpcionales = false): array
    {
        ...
        $datos = [
            ...
            'id_establecimiento' => (int) ($entrada['id_establecimiento'] ?? 0),
        ...
        if ($datos['apellido'] === '' || mb_strlen($datos['apellido']) > 100) {
            $errores['apellido'] = 'El apellido es obligatorio y admite hasta 100 caracteres.';
        }
        if (!preg_match('/^\d{11}$/', $datos['cuil'])) {
            $errores['cuil'] = 'El CUIL es obligatorio y debe contener exactamente 11 números.';
        }
        if ($datos['nombre_usuario'] === '' || mb_strlen($datos['nombre_usuario']) > 80) {
            $errores['nombre_usuario'] = 'El nombre de usuario es obligatorio y admite hasta 80 caracteres.';
        }
        if ($datos['correo'] !== null && !filter_var($datos['correo'], FILTER_VALIDATE_EMAIL)) {
            $errores['correo'] = 'El correo electrónico no tiene un formato válido.';
        }
        if ($datos['id_establecimiento'] < 1) {
            $errores['id_establecimiento'] = 'Selecciona un establecimiento.';
        }
        if ($datos['id_rol'] < 1) {
            $errores['id_rol'] = 'Selecciona un rol.';
        }
        if ($datos['id_estado_usuario'] < 1) {
            $errores['id_estado_usuario'] = 'Selecciona un estado.';
        }
        if ($usuarioId === null && strlen($datos['contrasena']) < 8) {
            $errores['contrasena'] = 'La contraseña debe contener al menos 8 caracteres.';
        }
        if ($usuarioId !== null && $datos['contrasena'] !== '' && strlen($datos['contrasena']) < 8) {
            $errores['contrasena'] = 'La nueva contraseña debe contener al menos 8 caracteres.';
        }
        if ((new UsuarioRepository())->existeNombreUsuario($datos['nombre_usuario'], $usuarioId)) {
        ...
        $permisos = array_values(array_filter(
            array_map('intval', (array) ($entrada['permisos'] ?? [])),
    ...
    private function procesarRespuestas(array $entrada, array &$errores, bool $opcionales): array
    {
        ...
        $resultado = [];
        for ($i = 0; $i < 3; $i++) {
            $normalizada = RespuestaSeguridadService::normalizar(
                (string) ($respuestasEntrada[$i] ?? '')
            );
            if ($normalizada === '') {
            ...
            $resultado[] = [
                'id_pregunta_seguridad' => (int) $preguntas[$i],
                'respuesta_normalizada' => $normalizada,
            ];
        }
        return $resultado;
    }
